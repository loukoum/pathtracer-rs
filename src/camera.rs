...
impl Camera {
	...
	pub fn generate_ray(&self, film_x: f32, film_y: f32) -> Ray {
		...
		Ray { origin: self.position
			, direction: (&(&self.left_edge + &(&self.horizontal_parallel * film_x)) + &(&self.vertical_parallel * (film_y - 0.5))).unit()
		}
	}
}
...
mod camera_tests {
    ...
    #[test]
    fn main_test() {
		let cam = Camera::new(std::f32::consts::PI / 2.0
				, 1.0
		...
		assert!(center_ray.direction == Vector3 { x: 0.0, y: 0.0, z: 1.0 });
	}
}
